...
export class OrderView extends FormView implements IOrderView {
    ...
    getActiveButton(): PaymentType {
        if (this._buttonOnline.classList.contains('button_alt-active')) return 'online';
        if (this._buttonUponReceipt.classList.contains('button_alt-active')) return 'upon receipt';
        return '';
    }
