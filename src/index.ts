...
// Отображения
const page = new PageView(ensureElement<HTMLElement>(settings.pageSelector), {
...
function handleGallerySetProducts(items: IProduct[]) {
	...
	for (const item of items) {
		...
		const card = new ProductInCatalogView(
			cloneTemplate(settings.productInCatalogTemplate),
			{
				onClick: () => {
...
function handlePreviewChange(item: IProduct) {
	const card = new ProductFullView(
		cloneTemplate(settings.productFullTemplate),
		{
