...
```
Класс `EventsEmiter` имплиментирует интерфейс `IEvents`.  
...
## Список событий
События осуществляются с помощью функции класса `EventsEmiter`. Элементы их перечисления (`Enum Events`):
- `SET_PRODUCTS_IN_GALLERY = 'gallery:add products'` - добавление списка продуктов в галерею;
- `ADD_PRODUCT_IN_GALLERY = 'gallery:add product'` - добавление продукта в галерею;
- `DELETE_PRODUCT_FROM_GALLERY = 'gallery:delete product'` - удаление продукта из галереи по идентификатору;
...
Также у галереи есть методы:   
1. `addProduct(product: IProduct): void` - метод добавления карточки в галерею;
2. `removeProduct(id: string): void` - метод удаления карточки из галереи.
```typescript
interface IGallery {
    products: IProduct[];
    total: number;
...
Интерфейс корзины.  
Единственное поле:
1. `products` - словарь с выбранными продуктами и их количеством, структура данных `Map` со строками-ключами и числами-значениями.

Методы:   
1. `addProduct(id: string): void` - метод добавления продукта в корзину;
2. `removeProduct(id: string): void` - метод удаления продукта из корзины;
3. `clearBucket() : void` - метод удаления всех продуктов из корзины.
```typescript
interface IBasket {
...
Методы:   
- Метод `addProduct(id: string): void` используется для того, чтобы добавить продукт в корзину. Этот метод принимает строку, идентификатор товара, и либо добавляет в словарь продуктов новый ключ со значением 1, либо, если в словаре уже есть ключ, равный идентификатору, значение данного ключа увеличивается на 1.
- Метод `removeProduct(id: string): void` используется для удаления товара из корзины. Его принцип действия очень похож на предыдущий метод с небольшими изменениями: если ключа, равного строке, переданной в метод, нет, то никаких дествий не происходит. Нельзя удалить того, чего нет. А вот если ключ найден, происходит анализ значения этого ключа: если оно равно единице, данный ключ удаляется из словаря, а вот если значение больше, то происходит лишь уменьшение этого значения на единицу.
- Метод `clearBucket(): void` очищает словарь выбранных продуктов. Этот метод будет использоваться после успешной оплаты заказа.   
...
### IView
Интерфейс для всех элементов отображения.
...
### ModalView
Абстрактный класс для работы с модальным окном.   
Поля:   
...
- `close(): void` - метод, который закрывает открытое модальное окно.
...
Этот класс наследует все методы и поля класса `ModalView` и содержит имплементацию интерфейса `IView`, то есть, использует и переопределяет метод `render`.
...
Этот класс наследует все методы и поля класса `ModalView` и содержит имплементацию интерфейса `IView`.
...
Этот класс наследует все методы и поля класса `ModalView`, поле класса `FormView` и содержит имплементацию интерфейса `IView`.
